 ...
 */
abstract class AbstractJSONTestDataProcessor implements DataProcessorInterface
{
    ...
    {
        if (array_values($element) !== $element) {
    ...
    private function checkFields(array $fields, string $level): void
    {
        foreach ($fields as $field) {
            if ((self::NAME_KEY !== $field) && (self::CHILDREN_KEY !== $field) && ('_' === $field[0])) {
                throw new DataParseException("Unknown service field $field at level $level.");
            }
            if (0 !== preg_match('/[^a-z_]/', $field)) {
